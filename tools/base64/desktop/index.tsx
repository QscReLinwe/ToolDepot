...
import type { Base64Input, Base64Result } from '../core/index.js';

export const Component: FC<ToolViewProps<Base64Input, Base64Result>> = ({ tool, initialInput }) => {
  const [text, setText] = useState(initialInput?.text ?? '');
  const [mode, setMode] = useState<Base64Input['mode']>(initialInput?.mode ?? 'encode');
  ...
  async function handleRun(): Promise<void> {
    setError('');
    setOutput('');
    const input: Base64Input = { text, mode };
